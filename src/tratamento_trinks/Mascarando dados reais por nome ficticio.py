...
try:
    ...
    dados = pd.read_csv('C:/Users/thoma/OneDrive/Área de Trabalho/salao - fios de luxo/Tratamento de dados/agendamentos_atualizado.csv', delimiter=',', encoding='utf-8')
    ...
    for i, nome in enumerate(dados['Cliente']):
        if nome in nomes_repetidos:
            dados.at[i, 'Cliente'] = nomes_repetidos[nome]
        else:
            novo_nome = f"Cliente {contador}"
            nomes_repetidos[nome] = novo_nome
            dados.at[i, 'Cliente'] = novo_nome
            contador += 1
